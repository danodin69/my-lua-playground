...
function updateUpgradeState(dt)
    spawner.timer = spawner.timer - dt
    if spawner.timer <= 0 then
        if spawner.wave < 3 then  -- Add max wave check
...
function love.update(dt)
    ...
    elseif game_state == "upgrade" then
        updateUpgradeState(dt)  -- Add this
    ...
    -- Note: No update needed for "gameover" state
    end
end
...
function love.keypressed(key)
    if game_state == "upgrade" then
        if key == "c" then  -- Continue without upgrading
